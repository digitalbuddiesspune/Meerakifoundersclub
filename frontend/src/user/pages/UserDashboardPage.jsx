    ...
    <>
      <div className="mb-7 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">Thursday, 24 April 2026 · Nagpur, Maharashtra</p>
        </div>
        <div className="flex items-center gap-2">
          <button type="button" className="h-10 w-10 rounded-xl border border-slate-200 bg-slate-50">
            🔍
          </button>
          <button type="button" className="relative h-10 w-10 rounded-xl border border-slate-200 bg-slate-50">
      ...
      <section className="mb-5 flex flex-wrap items-center gap-4 rounded-2xl border border-slate-200 bg-gradient-to-r from-amber-50 to-orange-50 p-4">
        <div className="text-2xl">⚡</div>
        <div className="flex-1">
          <p className="text-sm font-bold text-[#F0B429]">Unlock Premium Features</p>
          <p className="text-xs text-slate-500">Pitch Room, 1:1 mentor booking, priority support and more.</p>
        </div>
      ...
      <section className="mb-5 grid gap-4 rounded-3xl border border-slate-200 bg-gradient-to-r from-orange-50 to-white p-6 lg:grid-cols-[1fr_auto]">
        <div>
          ...
          <span className="mt-2 inline-flex rounded-full border border-slate-200 bg-[#F0B429]/10 px-3 py-1 text-xs font-bold uppercase tracking-wider text-[#F0B429]">
            ★ {authUser?.plan || 'Premium'}
          </span>
          <div className="mt-4 flex flex-wrap gap-6 text-xs text-slate-500">
            <p>
              <span className="block text-sm font-bold text-slate-900">Apr 2025</span>Member Since
            </p>
            <p>
              <span className="block text-sm font-bold text-slate-900">12 months</span>Active Streak
            </p>
            <p>
              <span className="block text-sm font-bold text-slate-900">#47</span>Rank in MFC
            </p>
          </div>
        </div>
        <div className="flex items-center justify-center">
          ...
          >
            <div className="flex h-[86px] w-[86px] flex-col items-center justify-center rounded-full bg-white">
              ...
              <p className="mt-1 text-[9px] uppercase tracking-[0.14em] text-slate-500">MFC Score</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mb-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <article className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
          ...
          <p className="text-xs text-slate-500">Services Availed</p>
          <span className="mt-2 inline-flex rounded-md bg-green-500/10 px-2 py-1 text-[11px] text-green-700">
            ↑ 2 this month
          </span>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
          ...
          <p className="text-xs text-slate-500">Documents Stored</p>
          <span className="mt-2 inline-flex rounded-md bg-blue-400/10 px-2 py-1 text-[11px] text-blue-700">
            3 new uploads
          </span>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
          ...
          <p className="text-xs text-slate-500">Founders Referred</p>
          <span className="mt-2 inline-flex rounded-md bg-green-500/10 px-2 py-1 text-[11px] text-green-700">
            ↑ ₹2,490 earned
          </span>
        </article>
      ...
      <section className="mb-5 flex items-center gap-4 rounded-2xl border border-blue-200 bg-blue-50 p-4">
        <p className="text-3xl">🚀</p>
        <div>
          <p className="text-sm font-bold text-blue-700">My Pitch Room</p>
          <p className="text-xs text-slate-500">Store your startup deck, share with investors & mentors.</p>
        </div>
        <button
          type="button"
          className="ml-auto rounded-lg border border-blue-200 bg-blue-50 px-4 py-2 text-xs font-bold text-blue-700"
        >
          Open →
        </button>
      </section>
      ...
      <section className="mb-5 grid gap-4 xl:grid-cols-[1.6fr_1fr]">
        <article className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
          ...
          {servicesAvailed.map((item) => (
            <div key={item.name} className="flex items-center gap-3 border-b border-slate-200 py-3 last:border-b-0">
              ...
              <span
                className={`ml-auto rounded-full px-3 py-1 text-[10px] font-bold uppercase ${
                  item.status === 'done' ? 'bg-green-500/10 text-green-700' : 'bg-[#E8621A]/15 text-[#E8621A]'
                }`}
              >
        ...
        <article className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
          ...
          {events.map((event) => (
            <div key={event.name} className="flex items-start gap-3 border-b border-slate-200 py-3 last:border-b-0">
              <div className="flex h-11 w-11 shrink-0 flex-col items-center justify-center rounded-lg border border-[#E8621A]/25 bg-[#E8621A]/10">
                ...
                <p className="text-[9px] uppercase tracking-wider text-slate-500">{event.mon}</p>
              </div>
              <div>
