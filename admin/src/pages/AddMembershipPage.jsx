    ...
    <section className="grid gap-5">
      <div className="rounded-3xl border border-slate-200 bg-white p-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h3 className="mt-1 text-2xl font-bold text-slate-900">{editingId ? "Edit Membership" : "Add Membership"}</h3>
            <p className="mt-2 text-sm leading-7 text-slate-600">{editingId ? "Update membership plan details." : "Create a new membership plan."}</p>
          </div>
        </div>
      </div>
      ...
      <form className="grid gap-5" onSubmit={handleSubmit}>
        <div className="rounded-3xl border border-slate-200 bg-white p-[22px]">
          <h4 className="mb-4 text-base font-semibold text-slate-900">Plan Details</h4>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-[22px]">
          <div className="mb-3 flex items-center justify-between">
            ...
            <button type="button" onClick={addFeature} className="rounded-xl border border-slate-200 bg-[#F0B429]/10 px-3 py-1.5 text-xs font-bold text-slate-900 hover:bg-[#F0B429]/20">+ Add Feature</button>
          </div>
            ...
            {form.features.map((feature, i) => (
              <div key={i} className="flex gap-2">
                ...
                {form.features.length > 1 && (
                  <button type="button" onClick={() => removeFeature(i)} className="rounded-xl border border-red-400/30 bg-red-50 px-3 text-xs font-semibold text-red-600 hover:bg-red-100">Remove</button>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => navigate("/admin/memberships")} className="rounded-2xl border border-slate-200 bg-white px-6 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-white/10">
            Cancel
          </button>
        </div>
      </form>
